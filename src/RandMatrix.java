...
public class RandMatrix {
	...
	public static boolean[][] filterZeroRows(boolean[][] m){
		...
		for(int i = 0; i<m.length; i++){
			...
			int j = 0;
			while (tmp && j < m[0].length) { // try to disprove that i is zero row
				if(m[i][j] == true) tmp = false;
				j++;
			}
			if (tmp){
	...
	public  static boolean[][] filterDoubleColumns(boolean[][] m){
		...
		boolean[][] filteredM = new boolean[m.length][m[0].length - columnsCopies.size()] ;
		int j = 0;
