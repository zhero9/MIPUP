...
public class WritterPhylogenyTree {
	...
	public WritterPhylogenyTree(boolean[][] orgMatrix, boolean[][] matrix, boolean[][] filteredMatrix ,int[] rows, String colNames,
			String[] rowNames, String path, Vector<Vector<Integer>> columnsCopies, Vector<String> rowN){
	...
	public void writePhylogenyTreeFile(String pathToMatrix, int tmp, double trashHold){/// for VAF files
		...
		try{
			...
			writer.write("GL"+"[label=\"GL\",shape=box,style=filled,fillcolor=white];");
			writer.newLine();
			...
			System.out.println("Error in writing .dot  file for phylogeny tree.");
			e.printStackTrace();
			return;
		} finally {
			try {
	...
	private boolean isSubset(int k, int t){
		...
		for(int i = 0; i< matrix[0].length;i++)
			if(!originalMatrix[t][i] && matrix[k][i])	tmp =false;
		return tmp;
	}
